...
const PHASE1_WIN: ChecklistItem[] = [
  ...
  { id:'p1w-1.1-5', subsection:'1.1', subsectionLabel:'System Information', priority:'high', executable:true,
    label:'Danh sách bản vá bảo mật (Hotfixes) đã cài',
    purpose:'Đối chiếu bản vá đã cài với CVE đã công bố — máy thiếu bản vá quan trọng là bề mặt tấn công dễ bị khai thác.',
    suggestedTools:['Get-HotFix','wmic qfe','WSUS','Nessus'],
    commands:['wmic qfe list brief /format:table','powershell -c "Get-HotFix | Sort-Object InstalledOn -Descending | Select-Object HotFixID,Description,InstalledOn -First 20 | Format-Table -AutoSize"'] },
  ...
  { id:'p1w-1.7-6', subsection:'1.7', subsectionLabel:'Windows Event Logs', priority:'high', executable:true,
    label:'Quick hunt — truy vấn nhanh Event ID quan trọng (4624/4625/4697/4104)',
    purpose:'Soi nhanh ngay tại chỗ (không cần xuất evtx): đăng nhập thành/bại, cài service mới (backdoor), script PowerShell đáng ngờ.',
    suggestedTools:['EventViewer','DeepBlueCLI','Hayabusa'],
    commands:['powershell -c "Get-WinEvent -FilterHashtable @{LogName=\'Security\';ID=4624,4625} -MaxEvents 50 | Select-Object TimeCreated,Id,@{N=\'User\';E={$_.Properties[5].Value}} | Format-Table -AutoSize"','powershell -c "Get-WinEvent -FilterHashtable @{LogName=\'System\';ID=4697} -EA SilentlyContinue | Select-Object TimeCreated,Id,Message | Format-List"','powershell -c "Get-WinEvent -FilterHashtable @{LogName=\'Microsoft-Windows-PowerShell/Operational\';ID=4104} -EA SilentlyContinue -MaxEvents 20 | Select-Object TimeCreated,Message | Format-List"'] },
  { id:'p1w-1.7-6', subsection:'1.7', subsectionLabel:'Windows Event Logs', priority:'high', executable:true,
...
const PHASE1_LIN: ChecklistItem[] = [
  ...
  { id:'p1l-1.4-5', subsection:'1.4', subsectionLabel:'Users & Sessions', priority:'high', executable:true,
    label:'Cấu hình an toàn SSH daemon (sshd_config)',
    purpose:'Kiểm tra SSH có cho root login / password auth không — cấu hình lỏng là cửa vào brute-force phổ biến.',
    suggestedTools:['Lynis','ssh-audit'],
    commands:['grep -Ei "^(PermitRootLogin|PasswordAuthentication|PubkeyAuthentication|PermitEmptyPasswords|MaxAuthTries)" /etc/ssh/sshd_config 2>/dev/null'] },
  { id:'p1l-1.4-6', subsection:'1.4', subsectionLabel:'Users & Sessions', priority:'high', executable:true,
    label:'Lịch sử đăng nhập thất bại (lastb)',
    purpose:'Đếm & xem các lần đăng nhập sai — bùng nổ failed login = dấu hiệu brute-force đang/đã diễn ra.',
    suggestedTools:['lastb','fail2ban'],
    commands:['lastb -50 2>/dev/null > failed_logins.txt','echo "--- Top IP brute-force ---" >> failed_logins.txt','lastb 2>/dev/null | awk "{print \\$3}" | sort | uniq -c | sort -rn | head >> failed_logins.txt'] },
