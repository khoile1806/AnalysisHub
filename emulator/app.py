...
def scan_memory(se):
    """YARA-scan the emulated memory — catches a payload the sample UNPACKED in
    ...
    try:
        seen, hits = set(), []
        for m in _YARA_RULES.match(data=bytes(blob)):
...
def _emulate_core(data):
    ...
    try:
        ...
        result["memory_yara"] = scan_memory(se)  # YARA on the UNPACKED memory
...
def run_authenticode(path):
    """Verify an Authenticode signature: is it signed, is the signature VALID
    (not tampered/broken), and who is the publisher. A valid signature from a real
    publisher is a strong benign signal; a broken/forged one is suspicious."""
    ...
    pub = ""
    m = re.search(r"Subject:.*?/CN=([^/\n]+)", out) or re.search(r"Subject:.*?/O=([^/\n]+)", out)
    if m:
        pub = m.group(1).strip()
    signed = ("signature verification" in low) or bool(pub)
    return {"signed": signed, "valid": valid, "publisher": pub}
...
def extract_dotnet_rat(data, dotnet=None):
    ...
    if not family and not (c2 or tg):
        return None
    if not family:
        family = ".NET RAT (generic)"
    # Require at least one C2-like indicator to avoid a family-name-only false hit.
    if not c2 and not tg:
        return None
    return {"family": family, "c2": c2, "keys": keys, "campaign": "",
            "raw": "heuristic from .NET user strings"}


CONFIG_EXTRACTORS = (extract_cobaltstrike, extract_remcos, extract_dotnet_rat)
    ...
    try:
        ...
        if len(work) > DEEPSTATIC_MAX_BYTES:
            return jsonify(capabilities=[], attck=[], floss_strings=[], signature=sig,
                           config=config, unpacked=unpacked, dotnet=dotnet,
        ...
        return jsonify(capabilities=capa["capabilities"], attck=capa["attck"],
                       ...
                       unpacked=unpacked, dotnet=dotnet)
    except Exception as e:
...
# Extracts a function list + pseudo-C for the largest functions so the AI can
# read and explain the binary's logic. radare2 is light + apt-installable (vs a
# ~1GB Ghidra install); swap in Ghidra headless later for deeper decompilation.
...
def run_radare(path):
    ...
    if not r2:
        return None
    try:
        out = subprocess.run([r2, "-q", "-A", "-e", "scr.color=0", "-c", "aflj", path],
                             capture_output=True, timeout=200).stdout.decode("utf-8", "replace").strip()
        funcs = json.loads(out.splitlines()[-1]) if out else []
    except Exception:
        return None
    cand = [f for f in funcs if f.get("size", 0) >= 16 and not str(f.get("name", "")).startswith(("sym.imp", "fcn.0000"))]
    cand.sort(key=lambda f: f.get("size", 0), reverse=True)
    cand = cand[:10]
    pseudo = ""
    if cand:
...
def reverse():
    ...
    try:
        res = run_radare(path)
        if res is None:
            return jsonify(error="radare2 not available", functions=[], pseudocode=""), 200
        return jsonify(res)
    except Exception as e:
...
def analyze_pdf(data):
    ...
    for m in re.findall(r"/JS\s*\(((?:[^()\\]|\\.){8,})\)", text)[:10]:
